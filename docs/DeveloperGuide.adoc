...
The date storing mechanism only begins to work when an instance of the `Person` class in implemented. An instance of the `Date` class is used to store the current date that the `Person` instance was created.
Such an implementation doesn't allow error when creating the field as there is no room for mistakes on the user's side. An example of such an implementation of this is the `dateBorrow` field of `Person`.
[source,java]
----
...
----
When the `Person` instance is created, the following line is called:
[source,java]
----
...
----
This would result in inconsistencies in the code. For example, an `Edit` command is implemented in such a way that it creates an `editedPerson`. This can be seen in the following sequence diagram: +
...
**Cons:** Is not as user friendly. As of now, there amount of fields is rather high and having to input another one is tedious for the users.
...
|`* * *` |debt collector |have a list of people who have cleared their debts |refer to their recorded info when they require my services again
...
|`*` |debt collector |randomly generate a contact from the link:#cleared[cleared] list(clients who have paid their debts) |choose to chat up a random contact to talk them into getting a loan
...
....
Refers to clients that have paid off all their debts (in this context)
....
